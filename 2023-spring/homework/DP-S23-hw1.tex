...
\item (\textbf{Node sensitivity}) Imagine we have a data set
  ...
  \item \emph{Income correlation}: How correlated are friends'
    ...
    private algorithm which, when $\eps=1$, with probability at least 0.9 approximates $g$ with additive error $\pm O(1/n)$
    on all graphs for which $\# (edges) \geq n^2/20$. (The constants 1
